...
async function getSearchResults(
  ...
  function textReplace(text) {
    ...
    const capitaliseWord = replacedText
      .split(" ")
  ...
  try {
    const request = {
      ...
      headers: {
        ...
        "X-Goog-FieldMask":
          "places.displayName,places.priceLevel,places.photos,places.googleMapsUri,places.shortFormattedAddress,places.rating,places.userRatingCount,places.types,places.currentOpeningHours,places.priceLevel,nextPageToken",
      },
      data: {
        textQuery: textQuery,
    ...
    if (nextPageToken) {
      request.data.pageToken = nextPageToken;
    }
    ...
    if (response.status === 200) {
      ...
      return [updatedData, nextPageToken];
    } else {
